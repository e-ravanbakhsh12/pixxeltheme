...
if ($max_num_pages > 1) : ?>
    ...
    <div class="pixxel-pagination flex w-full pt-4 md:pt-14 text-sm" page="<?php esc_attr_e($page_query) ?>" max-page="<?php esc_attr_e($max_num_pages) ?>">
        <div class="mx-auto flex items-start gap-1 md:gap-3 bg-white rounded-full py-2 px-6">
            <?php if ($page_query > 1) : ?>
                <div  class="page-item size-6 md:size-8 rounded-full flex-center cursor-pointer flex hover:bg-light-blue transition-all hover:text-white" page="<?php esc_attr_e($page_query - 1) ?>">
                ...
                <?php for ($i = 1; $i <= $max_num_pages; $i++) : ?>
                    <?php if ($max_num_pages > 6 && $page_query > ($pagination_offset + 2)) : ?>
                        ...
                        <?php if ($i == $pagination_offset - 2 || $i == $pagination_offset - 1) : ?>
                            <div class="page-item size-6 md:size-8 rounded-full flex-center cursor-pointer flex   transition-all  <?php echo $i == $page_query ? 'bg-blue-main text-white selected' : ' hover:bg-light-blue ' ?>" page="<?php esc_attr_e($i) ?>"><?php esc_attr_e($i) ?></div>
                        <?php endif ?>
                    <?php endif ?>
            ...
            <?php if ($page_query != $max_num_pages) : ?>
                <div class="page-item size-6 md:size-8 rounded-full flex-center cursor-pointer flex hover:bg-light-blue transition-all"  page="<?php esc_attr_e($page_query + 1) ?>">
