...
$class = $args['class'];
$max_num_pages = $args['max-num-pages'];
$page_query = $args['page-query'];
$pagination_offset = $args['pagination-offset'];
...
if ($max_num_pages > 1) : ?>
    ...
    <div class="labell-pagination flex w-full my-5 text-sm" page="<?php esc_attr_e($page_query) ?>" max-page="<?php esc_attr_e($max_num_pages) ?>">
        <div class="mx-auto flex items-start gap-1 md:gap-3">
            <?php if ($page_query > 1) : ?>
                <div  class="page-item size-9 flex-center cursor-pointer flex bg-bg  hover:bg-magenta transition-all hover:text-white" page="<?php esc_attr_e($page_query - 1) ?>">
                ...
                <?php for ($i = 1; $i <= $max_num_pages; $i++) : ?>
                    <?php if ($max_num_pages > 6 && $page_query > ($pagination_offset + 2)) : ?>
                        ...
                        <?php if ($i == $pagination_offset - 2 || $i == $pagination_offset - 1) : ?>
                            <div class="page-item size-9 flex-center cursor-pointer flex  hover:bg-magenta transition-all hover:text-white <?php echo $i == $page_query ? 'bg-magenta text-white selected' : 'bg-bg' ?>" page="<?php esc_attr_e($i) ?>"><?php esc_attr_e($i) ?></div>
                        <?php endif ?>
                    <?php endif ?>
